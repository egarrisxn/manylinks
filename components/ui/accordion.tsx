...
function AccordionItem({
  ...
  return (
    <AccordionPrimitive.Item
      ...
      className={cn(
        "rounded-base border-bordershadow-base overflow-hidden border-2 border-b",
        className
      )}
      {...props}
    />
  ...
  return (
    <AccordionPrimitive.Header className='flex'>
      <AccordionPrimitive.Trigger
        ...
        className={cn(
          "text-main-foreground border-border bg-background font-heading flex flex-1 items-center justify-between p-4 text-left text-base transition-all focus-visible:ring-[3px] disabled:pointer-events-none disabled:opacity-50 data-[state=open]:rounded-b-none data-[state=open]:border-b-2 [&[data-state=open]>svg]:rotate-180",
          className
        )}
        {...props}
      >
